<template>
  <div id="TableStudent">
    <div id="tableBox">
      <!--        输入框区-->
      <div>
        <span class="text" id="nameText">姓名:&nbsp;<input type="text" v-model="name" placeholder=" 点此输入姓名" style="z-index: 1" class="selectName"/></span>
        <span class="text">学号:&nbsp;<input type="text" v-model="number"  placeholder=" 点此输入学号" class="selectNumber"/></span>
        <span class="text">班级:&nbsp;<select id="choseClass" v-model="myClass">
          <option name="class" disabled>---选择班级---</option>
          <option v-for="className in className">{{className}}</option>
          </select></span>
      </div>
      ...
      <div>
        <table id="table">
          ...
          <tr>
            ...
            <th style="width: 300px;">学号</th>
            <th style="width: 150px;">性别</th>
            <th style="width: 300px;">生日</th>
          ...
          <tr v-for="(data,index) in currentData">
            ...
            <td class="birthday">{{data.birthday.split("T")[0]}}</td>
            <td class="myClass">{{data.myClass}}</td>
      ...
      <div id="addBoxBG" v-show="isShowAddBox">
        <div id="addBox">
          ...
          <div id="add-Box-Inputs">
            <h3>{{addBoxName}}</h3>
            <span class="addBoxInput">姓名：<input type="text" placeholder=" 在此输入姓名" class="addBoxName"/></span>
            <span class="addBoxInput">学号：<input type="text" placeholder=" 在此输入学号" class="addBoxNumber"/></span>
            <span class="addBoxInput">性别：<input type="text" placeholder=" 在此输入性别" class="addBoxSex"/></span>
            <span class="addBoxInput">生日：<input type="text" placeholder=" 在此输入生日" class="addBoxBirthday"/></span>
            <span class="addBoxInput">班级：<input type="text" placeholder=" 在此输入班级" class="addBoxClass"/></span>
          </div>
...
<script>
import {select} from "@/network/tableStudent";
import {selectClassName} from "@/network/tableStudent";
import {selectByCondition} from "@/network/tableStudent";
import {deleteData} from "@/network/tableStudent";
import {addData} from "@/network/tableStudent";
import {updateData} from "@/network/tableStudent";
import {batchDeleteData} from "@/network/tableStudent";
...
export default {
  ...
  methods: {
    ...
    selectByCondition () {
      ...
      //对取到的查询出来的数据进行处理
      selectByCondition(this.name, this.number, this.myClass).then(res => {
        // console.log(res);
    ...
    changShowAddBox() {
      ...
      document.querySelector(".addBoxNumber").value = ''
      document.querySelector(".addBoxSex").value = ''
      document.querySelector(".addBoxBirthday").value = ''
      document.querySelector(".addBoxClass").value = ''
    ...
    addOrUpdateData() {
      if (this.isAdd) {
        ...
        let number = document.querySelector(".addBoxNumber").value
        let sex = document.querySelector(".addBoxSex").value
        let birthday = document.querySelector(".addBoxBirthday").value
        let myClass = document.querySelector(".addBoxClass").value

        if (confirm("确认添加数据吗？")){
          addData(name,number,sex,birthday,myClass).then( res => {
            if (res.message == "新增成功"){
              alert("添加数据成功")
              document.querySelector(".addBoxName").value = ''
              document.querySelector(".addBoxNumber").value = ''
        ...
        let birthday = document.querySelector(".addBoxBirthday").value
        let myClass = document.querySelector(".addBoxClass").value
        updateData(this.updateId,name,number,sex,birthday,myClass).then( res => {
          // alert(this.updateId)
          // console.log(res);
          if (res.message == "修改成功") {
            alert("修改数据成功")
    ...
    editData(e) {
      ...
      let number = trElement.querySelector(".number").innerHTML;
      let sex = trElement.querySelector(".sex").innerHTML;
      let birthday = trElement.querySelector(".birthday").innerHTML;
      let myClass = trElement.querySelector(".myClass").innerHTML;


      this.updateId = id

      document.querySelector(".addBoxName").value = name
      document.querySelector(".addBoxNumber").value = number
      document.querySelector(".addBoxSex").value = sex
      document.querySelector(".addBoxBirthday").value = birthday
      document.querySelector(".addBoxClass").value = myClass
  ...
  beforeCreate() {
    ...
    //查找所有班级名称
    selectClassName().then(res => {
      for (let i = 0;i< res.obj.length ; i++ ){
        this.className.push(res.obj[i].myClass);
      }
    })
  }
}
...
.selectName,.selectNumber,#choseClass {
  margin-top: 10px;
  position: relative;
  width: 200px;
  height: 30px;
...
.selectName:focus,.selectNumber:focus,#choseClass:focus {
