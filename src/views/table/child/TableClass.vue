<template>
  <div id="TableStudent">
    <div id="tableBox">
      <!--        输入框区-->
      <div>
        <span class="text" id="nameText">班级:&nbsp;<input type="text" v-model="name" placeholder=" 点此输入班级名称" style="z-index: 1" class="selectName"/></span>
        <span class="text">专业:&nbsp;<input type="text" v-model="special"  placeholder=" 点此输入专业" class="selectDepartment"/></span>
        <span class="text">院系:&nbsp;<select id="choseDepartment" v-model="department">
          <option name="class" disabled>---选择院系---</option>
          <option v-for="departmentName in departmentName">{{departmentName}}</option>
          </select></span>
      </div>
      ...
      <div>
        <table id="table">
          ...
          <tr>
            ...
            <th style="width: 300px;">班级名称</th>
            <th style="width: 400px;">专业</th>
            <th style="width: 350px;">院系</th>
          ...
          <tr v-for="(data,index) in currentData">
            ...
            <td class="special">{{data.special}}</td>
      ...
      <div id="addBoxBG" v-show="isShowAddBox">
        <div id="addBox">
          ...
          <div id="add-Box-Inputs">
            <h3>{{addBoxName}}</h3>
            <span class="addBoxInput">班级：<input type="text" placeholder=" 在此输入班级" class="addBoxName"/></span>
            <span class="addBoxInput">专业：<input type="text" placeholder=" 在此输入专业" class="addBoxSpecial"/></span>
            <span class="addBoxInput">院系：<input type="text" placeholder=" 在此输入院系" class="addBoxDepartment"/></span>
          </div>
...
<script>
import {select} from "@/network/tableClass";
import {selectDepartment} from "@/network/tableClass";
import {selectByCondition} from "@/network/tableClass";
import {deleteData} from "@/network/tableClass";
import {addData} from "@/network/tableClass";
import {updateData} from "@/network/tableClass";
import {batchDeleteData} from "@/network/tableClass";
...
export default {
  ...
  methods: {
    ...
    selectByCondition () {
      ...
      //对取到的查询出来的数据进行处理
      selectByCondition(this.name, this.special, this.department).then(res => {
        // console.log(res);
    ...
    changShowAddBox() {
      ...
      document.querySelector(".addBoxSpecial").value = ''
      document.querySelector(".addBoxDepartment").value = ''
    ...
    addOrUpdateData() {
      if (this.isAdd) {
        ...
        let special = document.querySelector(".addBoxSpecial").value
        let department = document.querySelector(".addBoxDepartment").value


        if (confirm("确认添加数据吗？")){
          addData(name,special,department).then( res => {
            console.log(res);
            if (res.message == "新增成功"){
              alert("添加数据成功")
              document.querySelector(".addBoxName").value = ''
        ...
        let special = document.querySelector(".addBoxSpecial").value
        let department = document.querySelector(".addBoxDepartment").value
        updateData(this.updateId,name,special,department).then( res => {
          // alert(this.updateId)
          // console.log(res);
          if (res.message == "修改成功") {
            alert("修改数据成功")
    ...
    editData(e) {
      ...
      let special = trElement.querySelector(".special").innerHTML;
      let department = trElement.querySelector(".department").innerHTML;


      this.updateId = id

      document.querySelector(".addBoxName").value = name
      document.querySelector(".addBoxSpecial").value = special
      document.querySelector(".addBoxDepartment").value = department
  ...
  beforeCreate() {
    ...
    selectDepartment().then(res => {
      for (let i = 0;i< res.obj.length ; i++ ){
        this.departmentName.push(res.obj[i].department);
      }
    })
  }
}
...
.selectName,.selectDepartment,#choseDepartment {
  margin-top: 10px;
  position: relative;
  width: 200px;
  height: 30px;
...
.selectName:focus,.selectDepartment:focus,#choseDepartment:focus {
